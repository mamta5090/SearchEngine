...
export const ConnectionService = {
  async create(userId, data) {
    const { name, host, port, database_name, db_username, password, ssl } = data;
    const encrypted_password = encrypt(password);
    return ConnectionRepository.create({
      userId, name, host, port, database_name, db_username, encrypted_password, ssl,
    });
  },
  ...
  async list(userId, page = 1, limit = 20) {
    ...
    const { rows, total } = await ConnectionRepository.listByUser(userId, limit, offset);
    return {
      connections: rows,
      pagination: {
        total,
  ...
  async update(userId, connectionId, data) {
    ...
    const updated = await ConnectionRepository.update(connectionId, userId, fields);
    return updated;
  },
  ...
  async testConnection(userId, connectionId) {
    const conn = await ConnectionRepository.findByIdWithSecret(connectionId, userId);
    if (!conn) throw ApiError.notFound('Connection not found');
